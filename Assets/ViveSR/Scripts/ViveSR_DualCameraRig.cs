...
namespace Vive.Plugin.SR
{
    ...
    public class ViveSR_DualCameraRig : ViveSR_Module
    {
        ...
        */
        ...
        /// </summary>
        private float ViewCameraFrameRatio = 0.8f;

        /// <summary>
        /// Apply an additional local scale on the render planes
        /// to improve the feeling of distance of near objects in the pass through content.
        /// </summary>
        private const float RenderPlaneAdditionalScale = 0.98f;
        /// <summary>
        ...
        public override bool Initial()
        {
            ...
            if (ViveSR.FrameworkStatus == FrameworkStatus.WORKING)
            {
                ...
                if (DualCameraCalibration != null)
                {
                    ...
                    // Set the render plane parameters.
                    renderPlaneLeft.UndistortedImagePlaneWidth = renderTextureLeft.width;
                    renderPlaneLeft.UndistortedImagePlaneHeight = renderTextureLeft.height;
                    renderPlaneLeft.UndistortedImagePlaneCenterX = 0.5f * renderTextureLeft.width;
                    renderPlaneLeft.UndistortedImagePlaneCenterY = 0.5f * renderTextureLeft.height;
                    renderPlaneLeft.CameraFocalLength = GetFocalLength(DualCameraLeft);
                    renderPlaneRight.UndistortedImagePlaneWidth = renderTextureRight.width;
                    renderPlaneRight.UndistortedImagePlaneHeight = renderTextureRight.height;
                    renderPlaneRight.UndistortedImagePlaneCenterX = 0.5f * renderTextureRight.width;
                    renderPlaneRight.UndistortedImagePlaneCenterY = 0.5f * renderTextureRight.height;
                    renderPlaneRight.CameraFocalLength = GetFocalLength(DualCameraRight);

                    // Set an additional local scale on the render planes to improve the feeling of object distance.
                    renderPlaneLeft.SetAdditionalScale(RenderPlaneAdditionalScale);
                    renderPlaneRight.SetAdditionalScale(RenderPlaneAdditionalScale);
                ...
                if (TrackedCameraLeft != null)
                {
                    if (TrackedCameraLeft.ImagePlane != null)
                    {
                        ...
                        TrackedCameraLeft.ImagePlane.ImagePlaneInit = true;
                    }
                ...
                if (TrackedCameraRight != null)
                {
                    if (TrackedCameraRight.ImagePlane != null)
                    {
                        ...
                        TrackedCameraRight.ImagePlane.ImagePlaneInit = true;
                    }
                ...
                if(RenderCameraLeft!= null)
                {
                    ...
                    ViewCameraFrame.SetCorrectSize(RenderTextureLevels[0].Left.width, RenderTextureLevels[0].Left.height, (float)ViveSR_DualCameraImageCapture.FocalLengthLeft);
                    ViewCameraFrame.SetFrame(ViewCameraFrameRatio);
                ...
                if(RenderCameraRight != null)
                {
                    ...
                    ViewCameraFrame.SetCorrectSize(RenderTextureLevels[0].Right.width, RenderTextureLevels[0].Right.height, (float)ViveSR_DualCameraImageCapture.FocalLengthRight);
                    ViewCameraFrame.SetFrame(ViewCameraFrameRatio);
                }
        ...
        private void EnableCameras(bool enableSRWorksCameras, DualCameraDisplayMode mode)
        {
            ...
            DualCameraImageRenderer.enabled = enableSRWorksCameras;
        ...
        public void SetViewCameraFrame(float ratio)
        {
            ViewCameraFrameRatio = ratio;
            if (RenderCameraLeft != null)
            {
                ViveSR_ViewCameraFrame ViewCameraFrame = RenderCameraLeft.GetComponentInChildren<ViveSR_ViewCameraFrame>();
                ViewCameraFrame.SetFrame(ViewCameraFrameRatio);
